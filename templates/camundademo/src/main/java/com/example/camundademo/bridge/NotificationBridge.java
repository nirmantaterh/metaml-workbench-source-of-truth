...
/**
 * What the generated controllers call after an activity completes, to tell the other side.
 *
 * <p>The seam between Camunda-driven execution and RabbitMQ-carried communication. Never touches
 * the engine itself - Camunda owns process execution, this only reports on it.
 *
 * <p>The log line fires whether or not messaging is enabled, so "this side notified the other"
 * stays observable with no broker configured; publish is what actually carries the message.
 */
@Component
public class NotificationBridge {
    ...
    /**
     * Manufacturing finished an activity: tell the twin, and ask it to run QC.
     *
     * <p>Starts a new correlation id, propagated by the twin through the gateway and back, so the
    ...
    /**
     * The twin side finished an activity of its own: report it to manufacturing.
     *
     * <p>Distinct from the twin's automatic reply inside the QC chain - this is the twin's own
     * generated controller reporting a directly-invoked twin activity.
     */
    ...
    /**
     * Manufacturing needs machines acquired for an activity before it can proceed.
     *
     * <p>Not called by generated code yet - which activities need machines is a process-specific
     * decision (RedCollar), not something the generic generator can know. Exposed so the machines
     * flow has a real entry point once such a model exists.
     */
